...
EXPECTED_COLUMNS = {"CODGEO", "MED21", "TP6021", "NBPERSMENFISC21"}
CRITICAL_COLUMNS = ["CODGEO", "MED21", "TP6021"]
...
def test_table_has_rows() -> None:
    with ENGINE.connect() as conn:
        result = conn.execute(text("SELECT COUNT(*) FROM raw.insee_communes"))
...
def test_no_nulls_on_critical_columns(column: str) -> None:
    with ENGINE.connect() as conn:
        result = conn.execute(
            text(
                f"""
            SELECT COUNT(*) FROM raw.insee_communes WHERE "{column}" IS NULL
        """
            )
        )
        assert result.scalar() == 0
